// jQuery ellipse-box and lightening.
...
function log() {
   ...
   var z = $("#pizza");
   var zt = z.offset().top;
   var zl = z.offset().left;
   var zw = z.outerWidth();
   var zh = z.outerHeight();
   ...
   $("#log").html("box: " + el.toFixed() + "/" + et.toFixed() + " <br/>" 
	   + "pepper: " + pl.toFixed() + "/" + pt.toFixed() + " <br/>"
	   + "pizza: " + zl.toFixed() + "/" + zt.toFixed() );
}
...
function init() {
    ...
    canvas = document.getElementById('lightening-canvas');
    ...
    dragPoints.push(new DragPoint(pizza.offsetLeft, pizza.offsetTop));
    dragPoints.push(new DragPoint(ellipseBox.offsetLeft, ellipseBox.offsetTop));
...
// Taken from jQuery function: $("#MyElement").is(":visible")
